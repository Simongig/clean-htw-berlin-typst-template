...
#let titlepage(
  ...
  align(center, text(size: page-grid, fill: luma(80), [Abschlussarbeit\ zur Erlangung des akademischen Grades:]))
  if (type-of-thesis != none and type-of-thesis.len() > 0) {
    align(center, text(size: page-grid, weight: "bold", type-of-thesis))
    v(0.25 * page-grid)
  }

  align(center, text(size: page-grid, fill: luma(80), [an der]))
  // university
  text(weight: "bold", university + [ ] + university-location) 
  linebreak()
  ...
  place(
    ...
    grid(
      ...
      ..if ("company" in supervisor) {
        (
          align(top, text(weight: "bold", fill: luma(80), TITLEPAGE_COMPANY_SUPERVISOR.at(language))),
